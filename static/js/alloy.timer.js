Jx().$package(function(J){
	...
	var timeCall = function(){
		...
		if(promptText!=""){
			...
			alert(text.timeUp);
			// added by skyblue
			$.post('/timer/add_record', {
				'mission': taskNameEl.value, 
				'done': true
			}, function(e){
			});
		}
	...
	$E.on(startWorkButton,"click", function(e){
		if(!isTiming){
			if(taskNameEl.value !== text.pleaseStart && taskNameEl.value !== ""){
				// added by skyblue
				//if (!taskNameEl.value.match(/^#\d/)) {
				//	var tomatoData = J.json.parse(localStorage.getItem("tomatoData"));
				//	var len = Object.keys(tomatoData.taskList).length+1;
				//	taskNameEl.value = '#'+len+ ' ' + taskNameEl.value
				//}

				$.post('/timer/add_record', {'mission': taskNameEl.value}, function(e){
					return true;
				});
	...
	$E.on(taskNameEl, "click", function(e){
	...
	var start = function(){
		J.sound.init();
		sound = new J.sound();
		sound.load("/static/audio/ring.mp3",false);
